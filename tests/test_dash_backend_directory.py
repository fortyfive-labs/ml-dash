...
SERVER_URL = os.environ.get(
  "ML_DASH_SERVER_URL", "https://qwqdug4btp.us-east-1.awsapprunner.com"
)
...
class TestDashBackendDirectory:
  ...
  def test_nested_directory_structure(self, unique_namespace):
    ...
    # Create run and log data
    with exp.run():
      exp.params.set(model="bert-base-uncased", dataset="squad-v2", learning_rate=3e-5)

      for step in range(3):
        exp.metrics.log(
          step=step, train_loss=0.5 - (step * 0.1), val_accuracy=0.7 + (step * 0.05)
        )
  ...
  def test_multiple_experiments_same_directory(self, unique_namespace):
    ...
    for i in range(3):
      ...
      with exp.run():
        exp.params.set(run_number=i + 1, learning_rate=0.001 * (i + 1), batch_size=128)
  ...
  def test_all_operations_with_directory(self, unique_namespace):
    ...
    with exp.run():
      # Parameters
      exp.params.set(model="resnet50", epochs=10, batch_size=128, lr=0.001)
      ...
      exp.info("Epoch 1 completed", epoch=1, loss=0.5)
...
class TestDashBackendDirectoryMethods:
  ...
  def test_initialize_experiment_with_directory(self, unique_namespace):
    ...
    experiment = backend.initialize_experiment(
      description="Test initialization", tags=["backend", "test"]
    )
  ...
  def test_create_run_after_initialize(self, unique_namespace):
    ...
    run = backend.create_run(
      name="test-run-1", tags=["test"], metadata={"version": "1.0"}
    )
  ...
  def test_log_parameters_with_directory(self, unique_namespace):
    ...
    result = backend.log_parameters(
      parameters={"model": "resnet50", "learning_rate": 0.001, "batch_size": 128},
      operation="set",
    )
