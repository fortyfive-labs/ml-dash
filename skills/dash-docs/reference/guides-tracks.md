...
with Experiment("robotics/walker").run as experiment:
    for step in range(1000):
        ...
        # Control signals
        experiment.tracks("robot/control").append(
            step=step,
            motor1=ctrl[0],
            motor2=ctrl[1]
        )
        ...
        # Sensor readings
        experiment.tracks("robot/sensors").append(
            step=step,
            imu_x=imu[0],
            imu_y=imu[1],
            imu_z=imu[2],
...
with Experiment("rl/training").run as experiment:
    for episode in range(100):
        ...
        experiment.tracks("agent/states").append(
            episode=episode,
...
with Experiment("vision/tracking").run as experiment:
    for step in range(1000):
        # Track data
        experiment.tracks("agent/position").append(
            step=step,
            x=x,
            y=y
        )
...
with Experiment("rl/cartpole").run as experiment:
    ...
    for episode in range(100):
        ...
        while not done:
            ...
            experiment.tracks("agent/trajectory").append(
                episode=episode,
                step=step,
        ...
        experiment.tracks("agent/episodes").append(
            episode=episode,
...
experiment.tracks("agent/rewards").append(
    episode=episode,
    step=step,
    reward=reward
)
...
experiment.tracks("robot/joints/arm").append(data)
experiment.tracks("robot/joints/leg").append(data)
experiment.tracks("robot/sensors/imu").append(data)

# Less clear: Flat structure
experiment.tracks("arm_data").append(data)
experiment.tracks("leg_data").append(data)
```
...
experiment.tracks("sensor/temp").append(value=25.0
)

# With explicit timestamp
experiment.tracks("sensor/temp").append(
    timestamp=time.time(),
    data={"value": 25.0}
)
