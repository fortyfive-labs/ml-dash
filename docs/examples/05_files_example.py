...
def main():
  ...
  config = {"model": "resnet50", "learning_rate": 0.001, "batch_size": 32}
  ...
  with Experiment(
    name="files-demo", project="tutorials", local_path="./tutorial_data"
  ).run as experiment:
    ...
    result = experiment.files("config").save(
      "temp_files/config.json", description="Training configuration", tags=["config"]
    )
  ...
  print("  ls -lR tutorial_data/.dash/tutorials/files-demo/files/")
  print("=" * 60)
