...
exp = Experiment(
    ...
    tags=["resnet", "cifar10"]  # Optional tags for filtering
)
...
for run_id in range(5):
    exp = Experiment(
        ...
        tags=[f"run-{run_id}", "hyperparam-search"]
    )
...
exp = Experiment(
    ...
    tags=["resnet50", "cifar10", "transfer-learning"]
)
...
for idx, params in enumerate(combinations):
    ...
    exp = Experiment(
        ...
        remote="http://localhost:4000",
        tags=["hyperparam-search", "resnet50", f"combination-{idx + 1}"]
    )
...
def train_worker(rank, world_size, config):
    ...
    if rank == 0:
        exp = Experiment(
            ...
            remote="http://localhost:4000",
            tags=["distributed", f"{world_size}-gpu", "imagenet"]
        )
...
exp = Experiment(
    ...
    prefix="run-001",
    tags=["production", "v2.0", "critical", "imagenet", "resnet50"]
)
...
### 9. Use Remote Backend for Production
...
### 10. Use Descriptive Names and Documentation
...
exp = Experiment(
    ...
    """,
    tags=["resnet50", "imagenet", "fp16", "production"]
)
...
- **API Reference**: See [MINIMAL_API.md](MINIMAL_API.md) for complete API documentation
